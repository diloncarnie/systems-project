...
namespace MPU9250
{
    ...
    MPU9250::MPU9250(int bus, unsigned int address = 0x68)
    {
    ...
    int MPU9250::begin()
    {
        ...
        int imu_model = whoAmI();
        if ((imu_model != 113) && (imu_model != 115))
        {
            std::cerr << "[MPU9250 Address: 113/115 | MPU6500 Address: 112 ] WhoAmI Result --> " << imu_model << std::endl;
        }
        ...
        _gyroScale = 250.0f / 32767.5f * _d2r; // setting the gyro scale to 2000DPS
        _gyroRange = GYRO_RANGE_250DPS;
        // setting bandwidth to 20Hz as default
        ...
        _bandwidth = DLPF_BANDWIDTH_20HZ;
    ...
    int MPU9250::setSrd(unsigned int srd)
    {
        ...
        if (srd > 9)
        {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(100)); // long wait between AK8963 mode changes
    ...
    int MPU9250::readSensor()
    {
        ...
        if (readRegister(ACCEL_OUT, 21, _buffer) < 0)
        {
            return -1;
        }
...
the sensor should be rotated in a figure 8 motion until complete */
    ...
    int MPU9250::writeRegister(unsigned int reg, unsigned int data)
    {
        ...
        if (i2c_smbus_write_byte_data(i2c_file_, reg, data) < 0)
        {
    ...
    int MPU9250::readRegister(unsigned int reg, unsigned int count, unsigned int *buffer)
    {
        if (i2c_smbus_read_byte_data(i2c_file_,reg) < 0)
        {
    ...
    int MPU9250::writeAK8963Register(unsigned int subAddress, unsigned int data)
    {
        ...
        if (writeRegister(I2C_SLV0_CTRL, I2C_SLV0_EN | (unsigned int)1) < 0)
        {
            return -4;
        }
    ...
    int MPU9250::readAK8963Registers(unsigned int subAddress, unsigned int count, unsigned int *dest)
    {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(1)); // takes some time for these registers to fill
                                                                   // read the bytes off the MPU9250 EXT_SENS_DATA registers
        _status = readRegister(EXT_SENS_DATA_00, count, dest);
        return _status;
    }
    ...
    int MPU9250::whoAmI()
    {
        ...
        if (readRegister(WHO_AM_I, 1, _buffer) < 0)
        {
            return -1;
        }
